...
function createSessionCookie(req, res, payload) {
    ...
    else if(typeof payload === -1 ){
        jwtToken = deleteSessionJWT(payload);
    }else {
...
function deleteSessionJWT(payload){
    // ci-dessous, on met en place le cookie de session JWT :
    // 1/ on recupere notre clef privee
    const RSA_PRIVATE_KEY = fs.readFileSync('./keys/jwtRS256.key');
    ...
    const jwtToken = sessionJWT.sign(
        {
            payload: payload,
            midExp: Math.floor(0) + 0 // validité: 30mn
        },
        RSA_PRIVATE_KEY,
        {
            algorithm: 'RS256',
...
module.exports.deleteSessionJWT = deleteSessionJWT;
